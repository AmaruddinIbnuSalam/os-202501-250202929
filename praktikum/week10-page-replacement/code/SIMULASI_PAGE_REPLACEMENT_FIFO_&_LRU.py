print("SIMULASI PAGE REPLACEMENT DENGAN LOGIKA ALGORITMA FIFO & LRU")
print("Drag and drop refrence string.txt:")
file_path = input().strip()

# MEMBACA FILE REFERENCE.TXT
def read_refrence_string(file_path):
    with open(file_path, mode='r') as file:
        lines = file.readlines()
    
    reference_string = [int(x.strip()) for x in lines[0].split(',')]
    frames = int(lines[1].split('=')[1].strip())
